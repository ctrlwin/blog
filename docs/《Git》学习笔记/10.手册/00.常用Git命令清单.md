...
# 撤销commit提交
$ git reset --soft HEAD^
# 如果add也需要提交可以 --soft改成--hard
# HEAD^ 表示上一次commit，也可以写成HEAD~1。撤销两次就是HEAD~2
# --soft 不删除工作区代码，只撤销commit，不撤销add
...
## 十、暂存更改
``` bash
# 将所有未提交的修改（工作区和暂存区）保存至堆栈中，用于后续回复当前工作目录。
$ git stash
...
# 注：该命令将堆栈中最近保存的内容弹出并删除（栈是先进后出）
$ git stash pop

# 将堆栈的内容应用到当前目录，但不会删除内容
$ git stash apply
# 也可以指定恢复某一个内容
$ git stash apply + stash名称（如：stash@{1}）
...
# 后续想将stash中的内容恢复到当前工作目录时，如果是针对同一个文件的
# 修改（即便不是同行数据），那么可能会发生冲突，恢复失败，这里通过创
# 建新的分支来解决。可以用于解决stash中的内容和当前目录的内容发生冲
# 突的情景。发生冲突时，需手动解决冲突。
...
git checkout -b new_branch # 基于'branch'新建'new_branch'分支，并切换到该分支
git push --set-upstream origin new_branch # 推送新的分支，并设置本地分支跟踪新的远程分支
```

### 3.比较差异
```sh
# 显示出branch1和branch2中差异部分
